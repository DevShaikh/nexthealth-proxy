...
export const getAppointmentTypes = async (params: string) => {
  try {
    const { data }: AxiosResponse<NextHealthAPIResponse<AppointmentType[]>> = await externalApi.get(
      `/appointment_types${params}`
    );
...
export const createAppointment = async (params: string, payload: CreateAppointmentInput) => {
  try {
    const { data }: AxiosResponse<NextHealthAPIResponse<{ appt: Appointment }>> =
