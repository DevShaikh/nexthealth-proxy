...
export const getPatients = async (params: string) => {
  try {
    const { data }: AxiosResponse<NextHealthAPIResponse<{ patients: Patient[] }>> =
...
export const createPatient = async (params: string, payload: CreatePatientBodyInput) => {
  try {
    const { data }: AxiosResponse<NextHealthAPIResponse<Patient>> = await externalApi.post(
      `/patients${params}`,
      payload
    );
